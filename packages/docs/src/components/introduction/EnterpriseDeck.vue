        ...
        <template v-for="(item, i) in items" :key="i">
          <v-col cols="12" md="4">
            <v-responsive :min-height="mdAndUp ? 96 : undefined" class="mb-4">
              <h3 class="text-headline-medium font-weight-medium">
                {{ item.name }}
              </h3>
            ...
            <div class="mb-4">
              <span class="text-headline-medium font-weight-bold">${{ item.price }} USD</span>
              <span v-if="item.suffix" class="font-weight-medium text-medium-emphasis text-headline-small">{{ item.suffix }}</span>
            </div>
